  ...
  overlay.querySelector('#btn-store-save').addEventListener('click', async () => {
    ...
    try {
      ...
      overlay.querySelectorAll('[id^="acc-name-"]').forEach(input => {
        ...
        if (n) accountRows.push({ name: n, type: 'bank', balance: b })
      })
